 ...
 */
export async function getBaseModuleFiles(
  octokit: Octokit,
  context: PRContext,
  ...
  try {
    ...
    const bicepFiles = response.data.filter(
      (f: { type: string; name: string }) =>
        f.type === 'file' && f.name.toLowerCase().endsWith('.bicep')
    );

    for (const file of bicepFiles as Array<{ path: string; name: string }>) {
    ...
    log.debug(
      `Fetched ${Object.keys(moduleFiles).length} base module file(s) from ${repoRelativeDir}`
    );
    return moduleFiles;
  } catch (error) {
